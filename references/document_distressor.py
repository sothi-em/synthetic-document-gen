...
def create_mock_document():
    ...
    # Add title
    cv2.putText(img, "CONFIDENTIAL TEST DOCUMENT", (70, 100), font, 1.2, 0, 3, cv2.LINE_AA)
...
def apply_aging_effects(clean_img):
    ...
    for i, factor in enumerate([0.75, 0.82, 0.88]): # Differential darkening for brown tint
        paper[:, :, i] = (paper[:, :, i] * (1.0 - 0.35 * stain_mask_norm * factor)).astype(np.uint8)
    ...
    for i in range(3):
        ...
        dirty_img[:, :, i] = np.where(text_mask, int(30 * 0.85) + (paper[:, :, i] * 0.15).astype(np.uint8), paper[:, :, i])
